 ...
 */
function referentialValidate(ast) {
    const errors = [];
    const warnings = [];
    // quick maps
    ...
    function findTargetKey(t) {
        if (!t || !t.db || !t.schema || !t.table)
            return null;
    ...
    for (const m of ast.mappings || []) {
        if (m.from && m.from.source_id)
            usedSources.add(m.from.source_id);
    }
