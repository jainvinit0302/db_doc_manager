...
const CreateProject = () => {
  ...
  const handleFileUpload = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      // preserve existing mock parsing
        ...
        <div className="grid grid-cols-12 gap-6 min-h-[calc(100vh-8rem)]">
          ...
          <div className="col-span-6">
            <Card className="h-full">
              ...
              <CardContent className="space-y-6">
                {/* Parsed YAML Summary */}
                {/* Use parsedData + validationResult to show status and counts */}
                <div>
                    ...
                    {validationResult ? (
                      validationResult.valid ? (
                        <div className="flex items-center gap-1 text-green-600">
                          ...
                          <span className="ml-2 text-xs text-muted-foreground">Errors: {validationResult.errors.length}</span>
                          <span className="ml-2 text-xs text-muted-foreground">Warnings: {validationResult.warnings.length}</span>
                        </div>
                      ) : (
                      ...
                      parsedData.isValid ? (
                        <div className="flex items-center gap-1 text-green-600">
                          <Check className="w-4 h-4" />
                          <span className="text-sm">Looks Valid (client preview)</span>
                        </div>
                      ) : (
                        <div className="flex items-center gap-1 text-red-600">
                          <AlertCircle className="w-4 h-4" />
                          <span className="text-sm">Looks Invalid (client preview)</span>
                        </div>
                      )
                    ) : (
                      <div className="text-sm text-muted-foreground">No DSL parsed yet</div>
                    )}
                  </div>
                ...
                {validationResult ? (
                        ...
                        <ul className="mt-2 space-y-2">
                          {validationResult.errors.map((err, idx) => (
                            <li key={idx} className="p-2 bg-red-50 rounded">
                              <div className="text-xs text-red-700 font-medium">{err.type} {err.location ? <span className="text-muted-foreground">({err.location})</span> : null}</div>
                      ...
                      <div>
                        ...
                        <ul className="mt-2 space-y-2">
                          {validationResult.warnings.map((w, idx) => (
                            <li key={idx} className="p-2 bg-amber-50 rounded">
                              <div className="text-xs text-amber-700 font-medium">{w.type} {w.location ? <span className="text-muted-foreground">({w.location})</span> : null}</div>
                  ...
                  // if no validation result yet, show generated SQL or placeholder
                  <div>
                    <h3 className="font-medium mb-3 flex items-center gap-2">
