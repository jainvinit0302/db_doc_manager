...
const Dashboard = () => {
  ...
  const { loadProject, resetProject } = useProjectActions();
  ...
  const handleOpenProject = async (id: string) => {
    try {
      ...
      // Load project into context (NOT localStorage)
      loadProject({
        projectId: String(project.id),
        projectName: project.name,
        dslContent: project.dsl_content,
        metadata: project.metadata, // Contains validation state, parsedData
        isEditing: false, // Always read-only on open
      });
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-50 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container flex h-16 items-center justify-between px-4">
          ...
          <div className="flex items-center gap-3">
            <Button onClick={() => {
              resetProject(); // Reset context
              const nextProjectNum = projects.length + 1;
              navigate("/create-project", {
                state: { defaultName: `Untitled Project ${nextProjectNum}` }
              });
            }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredProjects.map((project) => (
              <ProjectCard
                ...
                onOpen={(id) => handleOpenProject(id)}
              />
            ))}
